...
- `npm run build:static` — regenerate the pre-rendered page content in `index.html` (hero stats, intro, problem cards, evidence, colophon) after a change to `data/`.
...
- `index.html` embeds the hero numbers, the intro paragraphs, the problem cards, the evidence prose, the colophon, and the static identity copy. Edit the data and run `npm run build:static` instead of the HTML; `validate:site` refuses drift.
